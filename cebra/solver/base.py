...
class Solver(abc.ABC, cebra.io.HasDevice):
    ...
    def maml_train(
        self,
        datas,  # List of task data (rat data)
        labels,  # Corresponding labels (rat labels)
        maml_steps: int = 1,  # Inner-loop steps for MAML
        maml_lr: float = 1e-3,  # Inner-loop learning rate
        ...
        for epoch in range(1, maml_steps + 1):
            ...
            for task_data, task_labels in zip(datas, labels):
                task_loader = cebra.data.Loader(task_data, task_labels, batch_size=len(task_data))
