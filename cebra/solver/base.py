...
class Solver(abc.ABC, cebra.io.HasDevice):
    ...
    def maml_fit(
        self,
        datas,  # List of tasks (each task should be a dataset)
        labels,  # Labels corresponding to each task
        maml_steps=5,  # Number of inner-loop steps (gradient updates per task)
        maml_lr=1e-3,  # Inner-loop learning rate
        save_frequency=500,  # Frequency of saving checkpoints
        logdir="./checkpoints",  # Log directory for saving checkpoints
        decode=False,  # Whether to perform decoding during training
    ):
        ...
        for epoch in range(1, maml_steps + 1):
            ...
            for task_data, task_labels in zip(datas, labels):
                task_loader = cebra.data.Loader(task_data, task_labels, batch_size=len(task_data))  # Create a data loader for each task
                ...
                for batch in task_loader:
                    stats = self.step(batch, model=model_copy)  # Call step function for gradient update
                    loss = stats['total']
